...
import Badge from '../Stats/badge';
import { BadgeTheme } from "../Stats/badge"
...
const Container = ({ frontmatter, image, className = 'card' }: ContainerProps) => {
  ...
  // Simple theme mapping based on category
  // components/card/container.tsx
  ...
  return (
    ...
    >
      ...
      <StatusBadge status={status} productTitle={title} />
      ...
      <div className={`flex flex-col flex-auto relative py-8 @lg/card:pb-[2.625rem] @lg/card:pt-12`}>
        <div className="flex flex-col gap-2.5 px-8 pt-[.8125rem] @lg/card:pt-2 @lg/card:gap-3 @lg/card:px-12">
          ...
          <h2
            className="p-0 m-0 leading-none text-balance font-display font-variation-bold hyphens-auto text-3xl @lg/card:text-5xl"
            id={`title-${id}`}
          >
