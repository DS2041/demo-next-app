...
interface NotepadSubcomponentProps {
  children: ReactNode;
  className?: string;
}

const Notepad = ({ children, className = '' }: NotepadProps) => {
  return <div className={`isolate relative ${className}`}>{children}</div>
}

const NotepadHeader = ({ children }: NotepadSubcomponentProps) => {
  return (
...
const NotepadBody = ({ children, className = '' }: NotepadSubcomponentProps) => {
  return (
    <div className={`bg-neutral-01-50 px-10 py-8 shadow-placed overflow-x-hidden rounded-bl-lg rounded-br-lg flex flex-col gap-4 relative z-10 ${className}`}>
      {children}
