...
// Import admin context instead of global
...
const BurnToken = ({ className = 'w-full', unique = 'footer' }) => {
    const { open } = useAppKit();
    const { address: userAddress } = useAccount()

    // Use admin context for transfer to burn functionality
    const {
        state,
        isTransferValid,
            ...
            <div className={`${className}`}>
                <div className="@container flex flex-col gap-10">
                                ...
                                {userAddress ? (
                                    state.ramiBalance !== undefined ? (
                                        <p className='flex justify-start items-center space-x-1'>
                                            <span className={`${hybridFormat(state.ramiBalance).fontSize} text-md opacity-80 leading-none flex items-center font-ui text-[#000000]`}>
                                                {hybridFormat(state.ramiBalance).formatted}
                    ...
                    <div className="flex flex-col gap-4 items-center justify-between">
                        {!userAddress && parseFloat(state.transferAmount) ? (
                            <Button
                                theme="dandelion"
                                disabled={true}
                            ...
                            <Button
                                onClick={handleTransferToBurn}
                                disabled={!isTransferValid || transaction.status === 'transferring'}
                                theme="dandelion"
                                className="button-dandelion select-none w-full flex-auto button-dandelion font-ui text-xl/tight lowercase text-center"
                            >
                        ...
                        <p className="m-0 p-0 text-fern-600 flex-1 text-center @sm:text-left">
                            Binance: 0x8530df905868e9b487ec6ad7753ef2d98f49dca6
                        </p>
                    </div>
                </div>
            </div>
