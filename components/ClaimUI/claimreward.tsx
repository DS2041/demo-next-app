...
const ClaimUiPage = ({ className = 'w-full', unique = 'footer' }) => {
          ...
          <div className="@container flex flex-col gap-6">
            <div className="flex flex-col">
              <div className='flex justify-between items-center'>
                ...
                {/* Ramicoin Balance */}
                {userAddress ? (
                  staking.usdtBalance !== undefined ? (
                    <p className='flex justify-start items-center space-x-1'>
                      <span className={`${hybridFormat(staking.usdtBalance).fontSize} text-md opacity-80 leading-none flex items-center font-ui text-[#000000]`}>
                        {hybridFormat(staking.usdtBalance).formatted}
              ...
              <div className="relative">
                <div
                  ...
                  <span className=''>
                    {userAddress ? (
                      staking.rewards !== undefined ? (
                        <div className='flex justify-end items-center space-x-1'>
                          <span className={`${formatRegular(staking.rewards)} whitespace-nowrap`}>
                            ${formatRegular(staking.rewards).formatted}
                          </span>
                        </div>
                      ) : (
                        <BlackSpinner />
          ...
          <div className="flex flex-col gap-4 items-center justify-between">
            ...
            {!userAddress ? (
              ...
              <Button
                theme="dandelion"
                onClick={handleClaim}
                disabled={transaction.status === 'claiming'}
                className="button-dandelion select-none w-full flex-auto button-dandelion font-ui text-xl/tight lowercase text-center"
              >
